...
const mealMonthMissCache = new Set<string>()
...
function invalidateMealPlanMonth(date: string) {
  ...
  if (!Number.isNaN(parsed.getTime())) {
    const cacheKey = mealMonthCacheKey(parsed.getFullYear(), parsed.getMonth())
    mealMonthCache.delete(cacheKey)
    mealMonthMissCache.delete(cacheKey)
  } else {
    mealMonthCache.clear()
    mealMonthMissCache.clear()
  }
}
...
export async function getDishes(): Promise<Dish[]> {
  ...
  const batches = await Promise.all(
    Array.from({ length: pageCount }, (_, page) =>
      getDishesPage({ skip: page * pageSize, limit: pageSize })
    )
  )
  return batches.flat()
}
...
/** 从已缓存分页中查找菜品，命中可省一次云端查询。 */
export function findCachedDishByName(name: string): Dish | null {
  for (const dishes of dishPageCache.values()) {
    const found = dishes.find((dish) => dish.name === name)
...
export async function getAllMealPlans(): Promise<MealPlan[]> {
  await waitForAppReady()
  const pageSize = DB_QUERY_LIMIT
  const countRes = await db.collection('meal_plans').count()
  const pageCount = Math.ceil(countRes.total / pageSize)
  const batches = await Promise.all(
    Array.from({ length: pageCount }, (_, page) =>
      db
  ...
  const plans = batches.flatMap((res) => res.data as MealPlan[])
...
export async function getMonthMealPlans(year: number, month: number): Promise<MealPlan[]> {
  ...
  if (mealMonthMissCache.has(cacheKey)) return []
  ...
  const request = (async () => {
    // 小程序端单次 get 最多返回 20 条，整月规划可能超过一页，需要分页。
    ...
    while (true) {
      const res = await db
        ...
        .orderBy('date', 'asc')
        .skip(page * DB_QUERY_LIMIT)
        .limit(DB_QUERY_LIMIT)
        .get()
      const batch = res.data as MealPlan[]
      plans.push(...batch)
      if (batch.length < DB_QUERY_LIMIT) break
      page++
    }
    rememberMealPlans(plans)
    if (plans.length === 0) {
      mealMonthMissCache.add(cacheKey)
    } else {
...
/** 同步读取已缓存的某月规划。返回 null 表示没缓存；空月返回 []。 */
...
export async function saveMealPlans(
  ...
  await Promise.all(plans.map(async (plan) => {
    ...
    upsertMealPlanCache({ ...plan, _id: String(addRes._id), createdAt: now, updatedAt: now } as MealPlan)
  }))
}
...
export async function deleteMealPlan(date: string): Promise<void> {
  ...
  if (res.data.length > 0) {
    await db.collection('meal_plans').doc(String(res.data[0]._id)).remove()
  }
