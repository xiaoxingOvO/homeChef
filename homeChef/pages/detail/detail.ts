...
import { getDishById, getDishByName, updateDish, deleteDish, getCategories, saveMealPlans, getMealPlans, getUserSettings, addDish } from '../../utils/db'
...
Page({
  ...
  async addToMeal(e: any) {
    ...
    const meals: MealSlot = existingPlan?.meals || {
      breakfast: [],
      lunch: [],
      dinner: [],
    }
    ...
    await saveMealPlans([{ date: today, meals }], existingPlan ? [existingPlan] : undefined)
    this.setData({ showMealModal: false })
