...
Page({
  ...
  renderDishes(allDishes: Array<Dish & { displayImage: string }>) {
  ...
  async batchAddToMeal(e: any) {
    ...
    const currentMeals: MealSlot = existingPlan?.meals || {
      breakfast: [],
      lunch: [],
      dinner: [],
    }
    ...
    await saveMealPlans([{ date: today, meals: currentMeals }], existingPlan ? [existingPlan] : undefined)
