...
Page({
  ...
  async loadPlanStats() {
    ...
    this.setData({ totalDays: Object.keys(plans).length })
    await this.loadRankingSummary(Object.values(plans))
  },

  async loadRankingSummary(plans: MealPlan[]) {
    try {
      ...
      const mostEaten =
        rankList.length > 0
          ...
          : '—'
      this.setData({ mostEaten, rankList })
    } catch (err) {
      console.error('加载最常吃统计失败:', err)
    }
  },
  ...
  filterDishManageDishes(allDishes: Dish[]) {
  ...
  onCategoryDragStart(e: any) {
    ...
    this.createSelectorQuery()
      ...
      .boundingClientRect((rects: any[]) => {
        ...
        categoryDragRects = rects.map((rect) => ({
          top: rect.top,
          bottom: rect.bottom,
        }))
