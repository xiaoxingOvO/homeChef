...
export function showToast(msg: string): void {
  ...
  if (page && page.setData) {
    page.setData({ toastMsg: msg, toastShow: true })
    clearTimeout(toastTimer)
    toastTimer = setTimeout(() => {
      if (page && page.setData) {
        page.setData({ toastShow: false })
      }
    }, 1500)
  }
}
...
export const DEFAULT_CATEGORIES: Category[] = [
  { name: '荤菜', isSystem: false },
  { name: '素菜', isSystem: false },
  { name: '汤', isSystem: false },
  { name: '主食', isSystem: false },
  { name: '凉菜', isSystem: false },
  { name: '小吃', isSystem: false },
